...
**Starting Point**: Existing Dome2 robot with core ROS2 functionality

**Hardware** (Dome2 platform):
...
**Dome2 Core Capabilities** (base platform):
...
**Explore Goal**: Build semantic mapping stack on top of Dome2 for autonomous exploration and object detection/cataloging
...
**Prerequisites** (provided by Dome2 base platform):
- **Hardware Interface Layer**: Motor control, sensor drivers (lidar, camera, IMU), odometry publishing
- **Base topics**: `/scan`, `/odom`, `/camera/rgb`, `/camera/depth`, `/cmd_vel`, `/imu` already available
- Explore semantic mapping stack builds on top of the working Dome2 platform
...
This semantic mapping application (Explore) assumes the Dome2 base platform is already installed and working:
- Dome2 motion control and odometry publishing on `/cmd_vel` and `/odom`
- Dome2 lidar driver publishing on `/scan`
- Dome2 camera driver publishing on `/camera/rgb` and `/camera/depth`
- Dome2 IMU publishing on `/imu`
...
**Note**: Hardware interface (motors, sensors, odometry, IMU, camera) is provided by Dome2 base platform and not included in the Explore stack.

**explore_bringup**: System startup and configuration
- Launch files for different system configurations
- Central configuration management
- Robot description (URDF)

**explore_msgs**: Message/Service/Action definitions
...
**explore_exploration**: Autonomous exploration
...
**explore_perception**: Object detection and tracking
...
**Launch**:
- `launch/exploration_full.launch.py` - Starts complete Explore semantic mapping system
- Individual component launches for testing (SLAM, Nav2, exploration separately)
- Assumes Dome2 base platform nodes are already running
